...
export default function Accounts() {
  ...
  const submitRename = async (id: string) => {
    try {
      const r = await fetch(`/api/accounts/${id}/rename`, {
        method: "PUT",
                  ...
                  <div>
                    {isRenaming ? (
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <button onClick={() => submitRename(a.id)} className="rounded-lg bg-ysbPurple px-3 py-2 text-sm font-semibold text-ysbYellow">
                          Save
                        </button>
                    ...
                    {a.type === "deriv" ? (
                      <>
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => submitEditSecrets(a.id, a.type)} className="rounded-lg bg-ysbPurple px-3 py-2 text-sm font-semibold text-ysbYellow">
                            Save
                          </button>
                      ...
                      <>
                        <div className="grid gap-2 md:grid-cols-3">
                          <div>
                            ...
                            <input className="w-full rounded-lg border border-border bg-background px-3 py-2" value={editServer} onChange={(e) => setEditServer(e.target.value)} />
                          </div>
                          <div>
                            <label className="block text-xs mb-1">Login</label>
                            <input className="w-full rounded-lg border border-border bg-background px-3 py-2" value={editLogin} onChange={(e) => setEditLogin(e.target.value)} />
                          </div>
                          <div>
                            <label className="block text-xs mb-1">Password</label>
                            <input className="w-full rounded-lg border border-border bg-background px-3 py-2" value={editPassword} onChange={(e) => setEditPassword(e.target.value)} type="password" />
                          </div>
                        </div>
