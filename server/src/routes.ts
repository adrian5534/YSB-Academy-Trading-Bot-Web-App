...
export function registerRoutes(app: express.Express, hub: WsHub) {
  ...
  // True global logout (revokes sessions everywhere) via service role
  router.post(
    "/api/auth/logout-all",
    requireUser,
    asyncRoute(async (req: AuthedRequest, res) => {
    ...
    asyncRoute(async (req: AuthedRequest, res) => {
      ...
      const r = await fetch(env.MT5_WORKER_URL.replace(/\/$/, "") + "/mt5/login", {
        ...
        headers: {
          ...
          "x-api-key": env.MT5_WORKER_API_KEY ?? "",
        },
  ...
  router.get(
    ...
    requireUser,
    asyncRoute(async (_req: AuthedRequest, res) => {
      const c = new DerivClient();
